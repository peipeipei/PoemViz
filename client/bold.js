...
Template.poem.events({
    ...
    //calls general addColor function
    ...
    'click .newBoldLayer':function(event){
        ...
        //starts the new bold layer with two default colors
        Colors.insert({
             poem_id:Session.get('currentPoem'),
             layer_id: layerID,
        ...
        //index only starts at two because two colors have already been assigned to the new bold layer
        ColorIndices.insert({
            poem_id:Session.get('currentPoem'),
            index: 2,
            layer: layerID
        });
        //go ahead and make the styles corresponding to those colors (and set the first as the default current style)
